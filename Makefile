...
prepare-venv:
	...
	$(if $(LIFX_LIB_BRANCH),$(VENV)/bin/pip install --force-reinstall "git+https://github.com/majamassarini/lifx-lib.git@$(LIFX_LIB_BRANCH)")
...
test:
	$(PYTHON) -m coverage run -m unittest discover \
		-s lifx_plugin/tests -p 'test*.py' -v
